...
NumberType primes_count_serial(NumberType n)
{
	// Two is special case
...
NumberType primes_count_parallel(NumberType n , int number_of_threads, NumberType grain_size)
{
    ...
    if( n>=3 )
	{
        ...
        parallel_reduce(SieveRange(s.multiples.m, n, s.multiples.m, grain_size), s, tbb::simple_partitioner());
        count += s.count;
    }
    return count;
}
